...
const EXTENSION_REPO: &str = "https://github.com/warpdotdev/agy-warp";
const EXTENSION_NAME: &str = "agy-warp";

// Keep in sync with the plugin version in warpdotdev/agy-warp.
const MINIMUM_PLUGIN_VERSION: &str = "1.0.0";

pub(super) struct AgyPluginManager {
    executor: LocalCommandExecutor,
    path_env_var: Option<String>,
}

impl AgyPluginManager {
    pub(super) fn new(
    ...
    async fn run_logged(&self, args: &[&str], log: &mut String) -> Result<(), PluginInstallError> {
        ...
        run_cli_command_logged("agy", args, &self.executor, env_vars, log).await
    }
}

#[async_trait]
impl CliAgentPluginManager for AgyPluginManager {
    ...
    fn is_installed(&self) -> bool {
        let Ok(extensions_dir) = agy_extensions_dir() else {
            return false;
        };
    ...
    async fn update(&self) -> Result<(), PluginInstallError> {
        ...
        let still_outdated = agy_extensions_dir()
            .ok()
    ...
    fn install_success_message(&self) -> &'static str {
        "Warp plugin installed. Please restart Antigravity CLI to activate."
    }

    fn update_success_message(&self) -> &'static str {
        "Warp plugin updated. Please restart Antigravity CLI to activate."
    }
...
static INSTALL_INSTRUCTIONS: LazyLock<PluginInstructions> = LazyLock::new(|| PluginInstructions {
    title: "Install Warp Plugin for Antigravity CLI",
    subtitle: "Run the following command, then restart Antigravity CLI.",
    steps: &[PluginInstructionStep {
        description: "Install the Warp extension",
        command:
            "agy extensions install https://github.com/warpdotdev/agy-warp --consent",
        executable: true,
        link: None,
    }],
    post_install_notes: &["Restart Antigravity CLI to activate the plugin."],
});

static UPDATE_INSTRUCTIONS: LazyLock<PluginInstructions> = LazyLock::new(|| PluginInstructions {
    title: "Update Warp Plugin for Antigravity CLI",
    subtitle: "Run the following command, then restart Antigravity CLI.",
    steps: &[PluginInstructionStep {
        description: "Update the Warp extension",
        command: "agy extensions update agy-warp",
        executable: true,
        link: None,
    }],
    post_install_notes: &["Restart Antigravity CLI to activate the update."],
});

fn check_installed(extensions_dir: &Path) -> bool {
    let manifest_path = extensions_dir
        .join(EXTENSION_NAME)
        .join("agy-extension.json");
...
/// Returns the path to `~/.antigravitycli/extensions`.
fn agy_extensions_dir() -> io::Result<PathBuf> {
    dirs::home_dir()
        .map(|home| home.join(".antigravitycli").join("extensions"))
        .ok_or_else(|| {
            io::Error::new(
